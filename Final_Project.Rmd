...
train_data %>% 
  select(committee_type_full, party_full, ) %>% 
  gather(var,val) %>% 
  ggplot(aes(val)) +
  geom_bar() +
  scale_y_log10() +
...
train_data %>% 
  ...
  scale_colour_manual(name="PAC Type",
                      ...
                      labels = c("Independent expenditure-only (Super PACs)", "PAC - Nonqualified", "PAC - Qualified", "PAC with non-contribution account - Nonqualified", "PAC with non-contribution account - Qualified")) +
  ggtitle("PAC Types Over Time By State") +
...
#Create new dummy variable for SuperPAC non-SuperPAC
for(i in new_data$CMTE_TP){
  if(i == "O"){
    print("1")
  }else{
    print("0")
...
#Create recipe
rcp <- 
  recipe(SuperPAC_yes ~ party + first_file_date + last_file_date + last_f1_date + CMTE_DSGN + CMTE_FILING_FREQ + CMTE_ST + TTL_CONTB + TTL_RECEIPTS + TTL_DISB + ORG_TP, train_data) %>% 
  step_dummy(all_nominal(),-all_outcomes()) %>%
  step_range(all_numeric()) %>% 
  step_rollimpute(all_numeric(), window = 3) %>% # Normalize scale
  prep()

#####missing party and ORG_TP when imputing

# Apply the recipe to the training and test data
train_data_processed <- bake(rcp,train_data)
test_data_processed <- bake(rcp,test_data) 
...
#Logit Model
mod_logit <-
  train(SuperPAC_yes ~ party + first_file_date + last_file_date + last_f1_date + CMTE_DSGN + CMTE_FILING_FREQ + CMTE_ST + TTL_CONTB + TTL_RECEIPTS + TTL_DISB + ORG_TP, train_data, # Training data 
...
mod_knn <-
  train(SuperPAC_yes ~ party + first_file_date + last_file_date + last_f1_date + CMTE_DSGN + CMTE_FILING_FREQ + CMTE_ST + TTL_CONTB + TTL_RECEIPTS + TTL_DISB + ORG_TP, train_data, # Equation (outcome and everything else)
