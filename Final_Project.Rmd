...
# KEEP ONLY THOSE IN RECIPES
x <- train_data2 %>% select(SuperPAC_yes, party, first_file_date, last_file_date, CMTE_DSGN, CMTE_FILING_FREQ, CMTE_ST, TTL_CONTB, TTL_RECEIPTS, TTL_DISB, ORG_TP)

# Turn party NAs into an actual factor
x <- x %>% mutate(party = ifelse(is.na(party), "NONE", party))
...
#  step_bagimpute(ORG_TP, party, options = list(nbagg = 5, keepX = FALSE)) %>%
  step_dummy(all_nominal(), -all_outcomes()) %>%
  step_range(all_numeric(), -first_file_date, -last_file_date) %>% 
  prep()
