...
rcp <-
  recipe(
    SuperPAC_yes ~ x, party, first_file_date, last_file_date, CMTE_DSGN, CMTE_FILING_FREQ, CMTE_ST, TTL_CONTB, TTL_RECEIPTS, TTL_DISB, ORG_TP, x
  ) %>%
#  step_bagimpute(ORG_TP, party, options = list(nbagg = 5, keepX = FALSE)) %>%
  step_dummy(all_nominal(), -all_outcomes()) %>%
  step_range(all_numeric(), -first_file_date, -last_file_date) %>% 
#  step_rollimpute(ORG_TP, party, window = 3) %>% # Normalize scale
  prep()
